...
int main(int argc, char *argv[]) {
  try {
    ...
    po.Register("output-format", &output_format, "Format of the output "
                "files [kaldi, htk]"); //std::cout<<output_format<<std::endl;
    po.Register("subtract-mean", &subtract_mean, "Subtract mean of each "
                "feature file [CMS]; not recommended to do it this way. "); //std::cout<<subtract_mean<<std::endl;
    ...
    po.Register("vtln-map", &vtln_map_rspecifier, "Map from utterance or "
                "speaker-id to vtln warp factor (rspecifier)"); //std::cout<<vtln_map_rspecifier<<std::endl;
    ...
    Mfcc mfcc(mfcc_opts);
    //std::cout<<"Mfcc num ceps: "<<mfcc_opts.num_ceps<<std::endl;
    //std::cout<<"Mfcc use energy: "<<mfcc_opts.use_energy<<std::endl;
    //std::cout<<"Mfcc energy floor: "<<mfcc_opts.energy_floor<<std::endl;
    //std::cout<<"Mfcc dimension: "<<mfcc.Dim()<<std::endl;

    SequentialTableReader<WaveHolder> reader(wav_rspecifier);
    //std::cout<<"Reader KEY: "<<reader.Key()<<std::endl;

    BaseFloatMatrixWriter kaldi_writer;  // typedef to TableWriter<something>.
    TableWriter<HtkMatrixHolder> htk_writer;
    //std::cout<<"utt2spk_rspecifier: "<<utt2spk_rspecifier<<std::endl;
    ...
    for (; !reader.Done(); reader.Next()) {
    	//std::cout<<"reader done: "<<reader.Done()<<std::endl;
      num_utts++;
      std::string utt = reader.Key(); //std::cout<<"utt: "<<reader.Key()<<std::endl;
      const WaveData &wave_data = reader.Value();
      	  //std::cout<<"duration: "<<wave_data.Duration()<<std::endl;
      	  //std::cout<<"sample freq: "<<wave_data.SampFreq()<<std::endl;
      	  //std::cout<<"data: "<<wave_data.Data()<<std::endl;
      ...
      if (vtln_map_rspecifier != "") {
        ...
        vtln_warp_local = vtln_map_reader.Value(utt); //std::cout<<"vtln_warp_local: "<<vtln_warp_local<<std::endl;
      } else {
        vtln_warp_local = vtln_warp; //std::cout<<"vtln_warp_local: "<<vtln_warp_local<<std::endl;
      }
      ...
      //std::cout<<"waveform: "<<waveform.Data()[1]<<std::endl;
      try {
      ...
      //std::cout<<"feature: "<<features.Index(0,0)<<std::endl;
      if (subtract_mean) {
